...
import type { UpdatePlanDTO, PlanType, ApplyScope, PlanDetailVO } from '@/types/subscription.types'

interface EditPlanDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  plan: PlanDetailVO | null
  onSubmit: (id: number, data: UpdatePlanDTO) => Promise<void>
}
...
export function EditPlanDialog({
  ...
  useEffect(() => {
    if (open && plan) {
      ...
      // Parse resourceLimits if exists
      if (plan.resourceLimits) {
        try {
          const limits = typeof plan.resourceLimits === 'string'
            ? JSON.parse(plan.resourceLimits)
            : plan.resourceLimits
          setResourceLimitsJson(JSON.stringify(limits, null, 2))
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-2">
                ...
                <NumberInput
                  minValue={1}
                  value={formData.minSeats ?? NaN}
                  onChange={(value) =>
                    setFormData({
                      ...formData,
                      minSeats: isNaN(value) ? undefined : value,
                    })
                  }
                  placeholder="默认1"
                />
              ...
              <div className="space-y-2">
                ...
                <NumberInput
                  minValue={1}
                  value={formData.maxSeats ?? NaN}
                  onChange={(value) =>
                    setFormData({
                      ...formData,
                      maxSeats: isNaN(value) ? undefined : value,
                    })
                  }
                  placeholder="留空表示无限制"
                />
              ...
              <div className="space-y-2">
                ...
                <NumberInput
                  ...
                  step={0.01}
                  formatOptions={{ minimumFractionDigits: 2, maximumFractionDigits: 2 }}
                  value={formData.seatPrice ?? NaN}
                  onChange={(value) =>
                    setFormData({
                      ...formData,
                      seatPrice: isNaN(value) ? undefined : value,
                    })
                  }
